import tkinter as tk
from tkinter import ttk
import sqlite3


class Main(tk.Frame):
    #класс основного окна
    def __init__(self, root):
        super().__init__(root)
        self.init_main()
        self.db = db
        self.view_records()

    def delete_records(self):
        #Удаление данных
        for selection_item in self.tree.selection():
            self.db.cur.execute("""DELETE FROM db WHERE id=?""",
                (self.tree.set(selection_item, '#1'),))
            self.db.conn.commit()
            self.view_records()

    def open_update_dialog(self):
        Update()
    
    def update_record(self, name, tel, email, zp):
        self.db.cur.execute("""UPDATE db SET name=?, tel=?, email=?, zp=? WHERE ID=?""",
            (name, tel, email, zp, self.tree.set(self.tree.selection()[0], '#1'),))
        self.db.conn.commit()
        self.view_records()

    def view_records(self):
        #Вывод данных их БД
        self.db.cur.execute("""SELECT * FROM db""")
        #Удаление из виджета таблицы
        [self.tree.delete(i) for i in self.tree.get_children()]
        #добавление в виджет информации из БД
        [self.tree.insert('', 'end', values=row) for row in self.db.cur.fetchall()]

    def records(self, name, tel, email, zp):
        #Соединение с бд
        self.db.insert_data(name, tel, email, zp)
        self.view_records()
    
    def open_dialog(self):
        #открытие дочерних окон
        Child()

    def init_main(self): 
        #хранит все виджеты на глав окне
        toolbar = tk.Frame(bg='#d7d8e0', bd=2)
        toolbar.pack(side=tk.TOP, fill=tk.X)
        
        #Добавление кнопки "добавить"
        self.add_img = tk.PhotoImage(file="./img/add.png") #./img/add.png
        btn_open_dialog = tk.Button(toolbar, bg='#d7d8e0', bd=0,
            image=self.add_img, command=self.open_dialog)
        #упаковка и выравнивание по левому краю
        btn_open_dialog.pack(side=tk.LEFT)

        #Добавление Treeview, основные колонки
        self.tree = ttk.Treeview(self, columns=('ID', 'name', 'tel', 'email', 'zp'),
            height=45, show='headings')
        #параметры колонкам
        self.tree.column('ID', width=30, anchor=tk.CENTER)
        self.tree.column('name', width=220, anchor=tk.CENTER)
        self.tree.column('tel', width=140, anchor=tk.CENTER)
        self.tree.column('email', width=140, anchor=tk.CENTER)
        self.tree.column('zp', width=150, anchor=tk.CENTER)

        #подписи колонок
        self.tree.heading('ID', text='ID')
        self.tree.heading('name', text='ФИО')
        self.tree.heading('tel', text='Телефон')
        self.tree.heading('email', text='E-mail')
        self.tree.heading('zp', text='Зарплата')

        #Упаковка
        self.tree.pack(side=tk.LEFT)

        #Кнопка "Обновить", внесения изменений
        self.update_img = tk.PhotoImage(file='./img/update.png')
        btn_edit_dialog = tk.Button(toolbar, bg='#d7d8e0', bd=0,
            image=self.update_img, command=self.open_update_dialog)
        btn_edit_dialog.pack(side=tk.LEFT)

        #Кнопка "удалить"
        self.delete_img = tk.PhotoImage(file='./img/delete.png')
        btn_delete = tk.Button(toolbar, bg='#d7d8e0', bd=0,
            image=self.delete_img, command=self.delete_records)
        btn_delete.pack(side=tk.LEFT)



class Child(tk.Toplevel):
    #класс дочерних окон
    def __init__(self):
        super().__init__(root)
        self.init_child()
        self.view = app

    def init_child(self):
        #Добавление дочернего окна
        self.title('Добавить')
        self.geometry('500x300')
        self.resizable(False, False)
        self.grab_set()
        self.focus_set()

        #Ввод данных для дочернего окна
        #подписи
        label_name = tk.Label(self, text='ФИО')
        label_name.place(x=50, y=50)
        label_select= tk.Label(self, text='Телефон')
        label_select.place(x=50, y=80)
        label_sum = tk.Label(self, text='E-mail')
        label_sum.place(x=50, y=110)
        label_zp= tk.Label(self, text='Зарплата')
        label_zp.place(x=50, y=140)

        #Добавление строки ввода и координат для name
        self.entry_name = ttk.Entry(self)
        self.entry_name.place(x=200, y=50)
        #Добавление строки ввода и координат для email
        self.entry_email = ttk.Entry(self)
        self.entry_email.place(x=200, y=80)
        #Добавление строки ввода и координат для tel
        self.entry_tel = ttk.Entry(self)
        self.entry_tel.place(x=200, y=110)
        #Добавление строки ввода и координат для zp
        self.entry_zp = ttk.Entry(self)
        self.entry_zp.place(x=200, y=140)

        #Кнопка закрытия дочернего окна
        self.btn_cancel = ttk.Button(self, text='Закрыть', command=self.destroy)
        self.btn_cancel.place(x=300, y=270)

        #Кнопка добавления
        self.btn_ok = ttk.Button(self, text='Добавить')
        self.btn_ok.place(x=220, y=270)
        #Срабатывание по ЛКМ
        self.btn_ok.bind('<Button-1>',
            lambda event: self.view.records(self.entry_name.get(),
                    self.entry_email.get(),
                    self.entry_tel.get(),
                    self.entry_zp.get()))
        

class Update(Child):
    #класс для внесения изменений
    def __init__(self):
        super().__init__()
        self.init_edit()
        self.view = app
        self.db = db
        self.default_data()

    def init_edit(self):
        #создание кнопки для внесения изменений
        self.title('Редактировать позицию')
        btn_edit = ttk.Button(self, text='Редактировать')
        btn_edit.place(x=205, y=170)
        btn_edit.bind('<Button-1>',
            lambda event: self.view.update_record(self.entry_name.get(),
                self.entry_email.get(), self.entry_tel.get(), self.entry_zp.get()))
        #закрываем окно редактирования
        btn_edit.bind('<Button-1>', lambda event: self.destroy(), add='+')
        self.btn_ok.destroy()

    def default_data(self):
        self.db.cur.execute("""SELECT * FROM db WHERE id=?""",
            (self.view.tree.set(self.view.tree.selection()[0], '#1'),))
        row = self.db.cur.fetchone()
        self.entry_name.insert(0, row[1])
        self.entry_email.insert(0, row[2])
        self.entry_tel.insert(0, row[3])
        self.entry_zp.insert(0, row[4])



class DB:
    #класс для создания БД
    def __init__(self):
        self.conn = sqlite3.connect('db.db')
        self.cur = self.conn.cursor()
        self.cur.execute("""CREATE TABLE IF NOT EXISTS db (
            id INTEGER PRIMARY KEY,
            name TEXT,
            tel TEXT,
            email TEXT,
            zp TEXT);""")
        self.conn.commit()

    def insert_data(self, name, tel, email, zp):
        self.cur.execute("""INSERT INTO db(name, tel, email, zp)
            VALUES (?, ?, ?, ?)""",
            (name, tel, email, zp))
        self.conn.commit()


#Условие проверяет где запускается приложение. Если локальный - запустится.
if __name__ == '__main__':
    #Запуск основного окна, добавление основного окна
    root = tk.Tk()
    db = DB()
    app = Main(root)
    app.pack()
    root.title('Список сотрудников компании')
    root.geometry('665x450')
    root.resizable(False, False)
    root.mainloop()
